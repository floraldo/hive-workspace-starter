...
For a missing requested CLI, show the official platform command from the reference and ask
for confirmation immediately before running it. Install user-level, never inside the
repository. Do not make authentication non-interactive; ask the user to complete sign-in in
the provider's own flow. Never handle tokens.
...
Validate by parsing the JSON, checking each compound dependency resolves to one task,
checking both launcher scripts for syntax where their shell is available, and confirming no
forbidden flag appears. Offer to start one task as a smoke test; launching the whole team is
a separate confirmation because it starts multiple interactive processes.
